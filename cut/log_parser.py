...
from cut.logger import get_logger
from cut import gcc_toolchain
from cut.build_action import BuildAction
...
INCLUDE_OPTIONS_MERGED = ["-iquote", "-[IF]", "-isystem", "-iprefix", "-iwithprefix", "-iwithprefixbefore"]
...
def filter_compiler_includes_extra_args(compiler_flags):
    ...
    pos = next((pos for pos, val in enumerate(compiler_flags) if val.startswith("--sysroot")), None)
    if pos is not None:
...
class ImplicitCompilerInfo:
    ...
    ImplicitInfoSpecifierKey = namedtuple("ImplicitInfoSpecifierKey", ["compiler", "language", "compiler_flags"])
    ...
    def is_executable_compiler(compiler):
        if compiler not in ImplicitCompilerInfo.compiler_isexecutable:
            ImplicitCompilerInfo.compiler_isexecutable[compiler] = find_executable(compiler) is not None
    ...
    def __get_compiler_err(cmd: List[str]) -> Optional[str]:
        ...
        try:
            ...
            # Python 3.8.
            LOG.error("Error during process execution: %s\n%s\n", " ".join(map(shlex.quote, cmd)), oerr.strerror)
    ...
    def get_compiler_includes(compiler, language, compiler_flags):
        ...
        # TODO: shlex.join(cmd) would be more elegant after upgrading to
        # Python 3.8.
        LOG.debug("Retrieving default includes via %s", " ".join(map(shlex.quote, cmd)))
...
#  error CC_FOUND_STANDARD_VER#98
        ...
        with tempfile.NamedTemporaryFile(
            mode="w+", suffix=(".c" if language == "c" else ".cpp"), encoding="utf-8"
        ) as source:

            with source.file as f:
                f.write(VERSION_C if language == "c" else VERSION_CPP)

            err = ImplicitCompilerInfo.__get_compiler_err([compiler, source.name])
        ...
        if standard:
            if standard == "94":
                ...
                standard = "-std=gnu" + ("" if language == "c" else "++") + standard

        return standard

    @staticmethod
    def dump_compiler_info(file_path: str):
        dumpable = {json.dumps(k): v for k, v in ImplicitCompilerInfo.compiler_info.items()}
    ...
    def load_compiler_info(file_path: str):
        ...
        for k, v in contents.items():
            k = json.loads(k)
            ICI.compiler_info[ICI.ImplicitInfoSpecifierKey(k[0], k[1], tuple(k[2]))] = v
    ...
    def set(details, compiler_info_file=None):
        ...
        def compiler_info_key(details):
            extra_opts = tuple(sorted(filter_compiler_includes_extra_args(details["analyzer_options"])))

            return ICI.ImplicitInfoSpecifierKey(details["compiler"], details["lang"], extra_opts)
        ...
        if compiler_info_file and os.path.exists(compiler_info_file):
            ...
            if iisk not in ICI.compiler_info:
                ICI.compiler_info[iisk] = {
                    "compiler_includes": ICI.get_compiler_includes(iisk.compiler, iisk.language, iisk.compiler_flags),
                    "compiler_standard": ICI.get_compiler_standard(iisk.compiler, iisk.language),
...
def __collect_transform_include_opts(flag_iterator, details):
    ...
    # not be necessary.
    flags_with_path = ["-I", "-idirafter", "-iquote", "-isysroot", "-isystem", "-sysroot", "--sysroot"]
...
def parse_options(
    ...
    if "arguments" in compilation_db_entry:
        ...
        details["original_command"] = " ".join([shlex.quote(x) for x in gcc_command])
    ...
    details["compiler"] = determine_compiler(gcc_command, ImplicitCompilerInfo.is_executable_compiler)
    ...
    compiler_version_info = ImplicitCompilerInfo.compiler_versions.get(details["compiler"], False)

    if not compiler_version_info and get_clangsa_version_func:

        # did not find in the cache yet
        try:
            compiler_version_info = get_clangsa_version_func(details["compiler"], env)
        except (subprocess.CalledProcessError, OSError) as cerr:
            LOG.error("Failed to get and parse version of: %s", details["compiler"])
            LOG.error(cerr)
            compiler_version_info = False

    ImplicitCompilerInfo.compiler_versions[details["compiler"]] = compiler_version_info

    using_same_clang_to_compile_and_analyze = False
    compiler_clang = ImplicitCompilerInfo.compiler_versions.get(details["compiler"])

    if compiler_clang:
        # Based on the version information the compiler is clang.
        flag_processors = clang_flag_collectors

        if analyzer_clang_version and compiler_clang.installed_dir == analyzer_clang_version.installed_dir:
    ...
    toolchain = gcc_toolchain.toolchain_in_args(details["analyzer_options"])
    ...
    if not keep_gcc_include_fixed:
        details["compiler_includes"] = list(filter(__is_not_include_fixed, details["compiler_includes"]))

    if not keep_gcc_intrin:
        details["compiler_includes"] = list(filter(__contains_no_intrinsic_headers, details["compiler_includes"]))
        ...
        for aopt in analyzer_options:
            ...
            if m:
                ...
                if os.path.isdir(value) and __contains_no_intrinsic_headers(value) or not os.path.isdir(value):
                    if together:
...
def process_response_file(response_file):
    ...
    sources = [
        opt for opt in options if not opt.startswith("-") and os.path.splitext(opt)[1].lower() in SOURCE_EXTENSIONS
    ]
            ...
            for opt in options:
                if opt.startswith("@"):
                    ...
                    if not os.path.exists(response_file):
                        LOG.warning("Response file '%s' does not exists.", response_file)
                        continue
...
def parse_unique_log(
    ...
    try:
        ...
        for entry in extend_compilation_database_entries(compilation_database):
            ...
            entry["file"] = os.path.normpath(os.path.join(entry["directory"], entry["file"]))
            ...
            if (
                analysis_skip_handlers
                and analysis_skip_handlers.should_skip(entry["file"])
                and (
                    not ctu_or_stats_enabled
                    or pre_analysis_skip_handlers
                    and pre_analysis_skip_handlers.should_skip(entry["file"])
                )
            ):
            ...
            action = parse_options(
                entry, compiler_info_file, keep_gcc_include_fixed, keep_gcc_intrin, None, env, analyzer_clang_version
            )
            ...
            if build_action_uniqueing == CompileActionUniqueingType.NONE:
                ...
                if action.source not in uniqued_build_actions:
                    ...
                    LOG.error(
                        "Build Action uniqueing failed" " as both '%s' and '%s'",
                        uniqued_build_actions[action.source].original_command,
                        action.original_command,
                    )
                    sys.exit(1)
            elif build_action_uniqueing == CompileActionUniqueingType.SOURCE_ALPHA:
                if action.source not in uniqued_build_actions:
                    uniqued_build_actions[action.source] = action
                elif action.output < uniqued_build_actions[action.source].output:
                    uniqued_build_actions[action.source] = action
            elif build_action_uniqueing == CompileActionUniqueingType.SOURCE_REGEX:
                LOG.debug("uniqueing regex")
                if action.source not in uniqued_build_actions:
                    uniqued_build_actions[action.source] = action
                elif uniqueing_re.match(action.original_command) and not uniqueing_re.match(
                    uniqued_build_actions[action.source].original_command
                ):
                    uniqued_build_actions[action.source] = action
                elif uniqueing_re.match(action.original_command) and uniqueing_re.match(
                    uniqued_build_actions[action.source].original_command
                ):
                    LOG.error(
                        "Build Action uniqueing failed as both \n %s" "\n and \n %s \n match regex pattern:%s",
                        uniqued_build_actions[action.source].original_command,
                        action.original_command,
                        compile_uniqueing,
                    )
        ...
        ImplicitCompilerInfo.dump_compiler_info(os.path.join(report_dir, "compiler_info.json"))
